...
// their inference. So this closure remembers the digest of the payload it
// most recently delivered successfully but has NOT yet finished auditing:
// on the next retry of the SAME still-queued item (identical bytes, since
...
// keeps failing (see spool.go's Run/forwardOldest) - so "the pending
// digest matches" can only mean "this is the same item, being retried",
// never a coincidental collision with some other item.
//
// The one remaining duplicate window is a process restart while an item is
// stuck in this state: the in-memory "already delivered" marker is lost,
// so the signal is redelivered (at most) once more on the next attempt
// after restart. That is bounded — one extra duplicate per restart during
// an audit outage — not the unbounded per-backoff-interval stream this
// guards against.
//
...
func auditingForward(deliver spool.ForwardFunc, logger *auditlog.Logger, destination string, health *auditHealth) spool.ForwardFunc {
	var mu sync.Mutex
	var pendingDigest string // digest of a payload already confirmed delivered, whose audit write hasn't succeeded yet ("" = nothing pending)

	return func(ctx context.Context, payload []byte) error {
		digest := hexDigest(payload)

		mu.Lock()
		alreadyDelivered := pendingDigest != "" && pendingDigest == digest
		mu.Unlock()
